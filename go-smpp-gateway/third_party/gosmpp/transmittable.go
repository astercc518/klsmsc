...
func newTransmittable(conn *Connection, settings Settings) *transmittable {
	t := &transmittable{
		settings:     settings,
		conn:         conn,
		// [KAOLACH PATCH] 发送端缓冲 1→256：deliver_sm_resp 由 receivable 读循环同步经此通道发出，
		// 缓冲为 1 时高峰出站 submit_sm 占满通道会阻塞读循环、令 ACK 延迟/丢失，引发上游对 DLR
		// 4 倍重传（详见事故分析）。增大缓冲让 ACK 不被出站流量堵死。
		input:        make(chan pdu.PDU, 256),
		aliveState:   Alive,
		pendingWrite: 0,
	}

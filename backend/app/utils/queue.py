...
class QueueManager:
    ...
    @staticmethod
    def queue_sms_batch(message_ids: list, queue: str = 'sms_send_smpp') -> bool:
        """
        批量投递 SMPP 发送任务（窗口化发送）
        ...
        for attempt in range(_SMS_QUEUE_RETRIES):
            try:
                ...
                logger.info(
                    f"SMPP 批量任务已入队: {len(message_ids)} 条, task_id: {task.id}"
                    + (f" (第{attempt + 1}次尝试)" if attempt else "")
                )
                return True
            except Exception as e:
                last_err = e
                logger.warning(
                    f"SMPP 批量入队失败将重试: {len(message_ids)} 条, "
        ...
        logger.error(
            f"SMPP 批量入队最终失败: {len(message_ids)} 条, 错误: {str(last_err)}",
            exc_info=last_err,
        )
        return False
